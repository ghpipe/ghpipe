...
**本环境已实测通过（2026-09-13，级别 L1）**：用**不继承父上下文**的独立上下文派发子 agent，任务只写在 `AGENTS.md` 指向的 `docs/TASK.md` 里，子 agent 能正确回报只存在于简报中的 nonce，并回答看不到父会话的其它 nonce（A1/A2/A3 全过）。**派发消息字段不可靠，不要用它作为任务来源。**

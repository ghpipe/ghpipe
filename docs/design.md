...
>
> **2026-09-13 实测：A1/A2/A3 全部通过，级别 L1。** 做法是用**不继承父上下文**的独立上下文派发子 agent，任务只写在仓库自带的 `AGENTS.md` 指向的 `docs/TASK.md`（未跟踪）里；子 agent 正确回报了只存在于该简报中的 nonce，并回答"看不到其它 nonce"。结论：**本工具支持独立上下文的原生子 agent；此前失效的只是"靠派发消息字段投递任务"这一种用法**。
>
> **有效配方（已验证）**：①用独立上下文派发（不继承父会话）；②任务正文写进 `docs/TASK.md`，并在 `AGENTS.md` 写明"子 agent 先读它"；③派发消息只作补充，不作为任务来源。
>

...
> **落地状态**：探针目前靠人工执行（`doctor --for handoff --probe-host` 见 G48）。
>
> **2026-09-13 实测：工具能派发独立上下文子 agent（A2/A3 成立），但"任务只来自 Issue"这一条在本环境当前不成立**——子 agent 沙箱无网络（`gh`/`api.github.com` 不可达），因此读不到 Issue；而派发消息字段也不会进入子 agent 上下文（子 agent 明确回报"没有收到任务"）。
>
> **由此得到的判据**：能不能用原生子 agent 承担开发/验收，**取决于"任务标识能否送达 + 子 agent 能否读到 Issue"**，而不是"工具有没有子 agent"。当前本环境的组合是"能派发、不能投递正文、不能读 Issue" → 按 §6.8.3 记为 `no_task_delivery`；按 §6.8.4 走降级（独立会话/进程或人工派发），并在账本记录 `independence`。
>
> **禁止的构造**：用本地文件（如 `docs/TASK.md`）承载任务正文——那会造出第二份任务真相，与「Issue 是唯一载体」冲突。此前自举中短暂用过这种办法，已废弃。
>
...
##### 6.8.5 派发通道：任务只来自 Issue/PR

**任务唯一载体是 GitHub Issue/PR，不允许任何本地第二份任务文档。** 这条是硬约束，理由与「GitHub 是账本、不建第二套状态」完全一致：一旦派发时另写一份任务说明，它就会与 Issue 漂移，出现"两份任务真相"，也让"谁定义了范围"变得不可审计。

派发时只传**任务标识**，不传任务正文：任务标识写进子 agent 的任务名/分支名（形如 `review-issue-8` / `ghpipe/issue-8`），子 agent 自己去读对应 Issue；仓库根的 `AGENTS.md` 只承载**通用约定**（怎么取标识、怎么读 Issue、读不到怎么办、禁止事项），不承载任何具体任务内容。

各种工具的投递方式不同（Codex 可用任务名 + 自定义 agent 指令；Claude Code 用 subagent 定义文件 + `description`；OpenCode 用配置 + `@` 点名），但**通道差异只影响"怎么把任务标识交给子 agent"，不影响"任务内容来自 Issue"**。

无法把标识送达、或子 agent 读不到 Issue（无网络/认证失败）时：子 agent 必须回复 `NO_TASK_SOURCE: <原因>` 并停止；调度者按 §6.8.3 记为 `no_task_delivery`，**不得**改用本地文件充当任务来源，而应按 §6.8.4 选择降级形态（独立会话/进程、或人工派发），并在账本里记录独立性来源。

##### 6.8.6 执行模式：单任务串行（硬规则）
...
| 扁平派发 | 子 agent **不得再派生下级 agent**；调度者直接派发 Developer 与 Reviewer，层级不超过 1 层（此前出现的三层嵌套链把并发槽占满，直接导致验收无法派发） |
| 固定顺序 | 同一任务按 §10.9 的顺序推进：判定 → 开发 → 分支 CI 三平台 → 独立验收 → 合并 → 收尾；任一步未结束不开启下一步 |
| 探针也串行 | 三 nonce 探针必须**先派 B、等它回报，再派 A**；并行派发会让 A2 假通过（§6.8.3） |
| 例外 | 只有"同一任务的开发与验收之间等待外部 CI"这一种等待可以并存于时间轴上，但它们也不是同时进行的工作；不得同时推进两个不同任务 |
| 违反处置 | 发现并行：立即停止新派发 → 等在办任务结束 → 把并行事实记入账本（Issue 评论 + `execution log`）→ `status` 报告该异常 |

这条规则的动机来自自举实践：并行/嵌套派发不仅让验收槽位被占满，还产生了"谁是产出者、谁是验收者"的账本混乱（同一分支上出现多个 agent 的提交与草稿）。
...
> **执行模式**：本循环**单任务串行**——同一时刻只推进一个任务，一步未结束不开启下一步；子 agent 不得再派生下级 agent（§6.8.6）。
